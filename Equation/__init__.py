...
def load():
    ...
    if not load.loaded:
        ...
        for file in os.listdir(dirname):
            ...
            if not plugin_file.lower().startswith(
                "equation_", 0, 9
            ) or extension.lower() not in [".py", ".pyc"]:
                continue
            if plugin_file not in plugins_loaded:
                ...
                try:
                    ...
                    fulltrace = "".join(
                        traceback.format_exception(errtype, errinfo, errtrace)[1:]
                    )
                    sys.stderr.write(
                        "Was unable to load {0:s}: {1:s}\nTraceback:\n{2:s}\n".format(
                            plugin_file, errinfo, fulltrace
                        )
                    )
                    continue
                if not hasattr(plugin_script, "equation_extend"):
                    sys.stderr.write(
                        "The plugin '{0:s}' from file '{1:s}' is invalid because its missing the attribute 'equation_extend'\n".format(
                            plugin_file,
                            (dirname.rstrip("/") + "/" + plugin_file + extension),
                        )
                    )
                    continue
